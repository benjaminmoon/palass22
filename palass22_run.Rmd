...
data <-
  ...
  write_tsv(paste0(dirs$pyrate, "ichthyosaurs.tsv"))
...
extract.ages.pbdb(
  file = paste0(dirs$pyrate, "ichthyosaurs.tsv"),
  sep = "\t",
...
```{r species_plots}
list_age <- list.files(".output/pyrate/pyrate_mcmc_logs/combined/", pattern = "TPP-ages_mcmc")
mcmc_log <- purrr::map(list_age, read_tsv) |>
  purrr::map(select, ends_with("_TS") | ends_with("_TE")) |>
  purrr::map_dfr(slice_tail, prop = 0.9) |>
  rename_with(~ str_replace_all(.x, "([[:alpha:]_])_(TS|TE)$", "\\2\\1"))
taxon_ages_samples <- mcmc_log |>
  pivot_longer(
    everything(),
    names_to = c("age_type", "taxon"), names_pattern = "(TS|TE)_([[:alpha:]_]+)",
    values_to = "age"
  )
taxon_age_samples_plot <- taxon_age_samples |>
  ggplot(aes(x = age, fill = type)) +
    geom_histogram(bins = 150)
taxon_age_samples_plot +
    facet_wrap(vars(sort(taxon, age, decreasing = TRUE))) +
    scale_x_reverse()
ggsave(file = "taxon_ages.pdf", width = 50, height = 30, limitsize = FALSE)
taxon_age_samples |>
  ggplot(aes(x = age, fill = type)) +
    geom_histogram(bins = 150) +
    facet_wrap(vars(sort(taxon, age, decreasing = TRUE)), ncol = 1) +
    scale_x_reverse()
ggsave(file = "taxon_ages_tall.pdf", width = 7, height = 300, limitsize = FALSE)
```
...
pdf(paste0(dirs$figs, "outgroup_phylogeny.pdf"), width = 5, height = 3)
plot(ladderize(outgroup_topology), no.margin = TRUE, cex = 0.5)
dev.off()
...
pdf(paste0(dirs$figs, "supertree_phylogeny.pdf"), width = 5, height = 3)
plot(ladderize(moon_supertrees[[1]]), no.margin = TRUE, cex = 0.2)
dev.off()
```
...
ggplot() +
  geom_sf(data = grat_wintri, colour = "grey60", size = 0.50/.pt) +
  geom_sf(data = world_map, inherit.aes = FALSE, size = 0.25/.pt) +
  geom_sf(data = map_data, mapping = aes(colour = max_ma), size = 1) +
...
# Occurrence Birth–Death Process for Diversification


```{r ofbd_occurrences}
# use this later for occurrence version
# fbd_dates <- data |>
#   mutate(
#     max = max_ma - minimum_age,
#     min = min_ma - minimum_age
#   )

# fbd_dates |>
#   select(min, max) |>
#   write_tsv(paste0(dirs$revbayes, "ichthyosaur_occurrences.tsv"))

minimum_age <- min(data$min_ma)
maximum_age <- 251.9

stages <- readLines(paste0(dirs$pyrate, "stages")) |>
  as.numeric()
stages <- stages[stages >= minimum_age]
substages <- readLines(paste0(dirs$pyrate, "substages")) |>
  as.numeric()
substages <- substages[substages >= minimum_age]

# combine multiple log files into one

output_revbayes_skyline_runfiles(occurrences = data, bins = stages, output_prefix = paste0(dirs$revbayes, "stages"), template = paste0(dirs$revbayes, "fbdrmatrix_stages_template.Rev"), replicates = 10)
output_revbayes_skyline_runfiles(occurrences = data, bins = substages, output_prefix = paste0(dirs$revbayes, "substages"), template = paste0(dirs$revbayes, "fbdrmatrix_substages_template.Rev"), replicates = 10)

old_par <- par()

combine_revbayes_logs <- function(path = "./output/revbayes/output", match) {
  # Combines logs from multiple RevBayes diversification runs and outputs a single file.
  #
  # Args:
  #   path (character): string to location of the log files.
  #   match (character): string within the file name to match. This matches files of the form '$match_\\d+_$rate'.
  #
  # Returns:
  #   Writes combined log files for each of extinction, speciation, and sampling rates and times (six files).
  logfiles <- c("extinction_rates.log", "sampling_rates.log", "speciation_rates.log", "extinction_times.log", "speciation_times.log", "sampling_times.log")
  for (rate in logfiles) {
    files <- list.files(path = path, pattern = paste0("^", match, "_\\d+_", rate), full.names = TRUE)
    lst <- lapply(files, read.table, sep = "\t", header = TRUE)
    lst <- do.call(rbind, lst)
    write.table(lst, file = paste0(path, "/combined_", match, "_", rate), sep = "\t")
  }
}

combine_revbayes_logs(match = "stages")
combine_revbayes_logs(match = "substages")

stage_rate <- compute_revbayes_rates(paste0(dirs$revbayes, "output/combined_"), match = "ages")

cairo_pdf(paste0(dirs$figs, "skyline_stages.pdf"), width = 7, height = 12)
plot_revbayes_rates(stage_rate, offset = minimum_age, max_age = maximum_age)
dev.off()

substage_rate <- compute_revbayes_rates(paste0(dirs$revbayes, "output/combined_"), match = "substages")

cairo_pdf(paste0(dirs$figs, "skyline_substages.pdf"), width = 7, height = 12)
plot_revbayes_rates(substage_rate, offset = minimum_age, max_age = maximum_age)
dev.off()
```
...
offset <- minimum_age
max_age <- maximum_age
div_rates <- c("Speciation rate", "Extinction rate", "Net diversification rate", "Relative extinction rate", "Fossilization rate")
...
cairo_pdf(file = paste0(dirs$figs, "substage_rb_pres_plots.pdf"), width = 8, height = 5, onefile = TRUE)
par(mar = c(4, 5, 0, 0) + 0.25)
for (rate_type in div_rates) {
  ...
  plot(x = rows$end_times, y = rows$median,
    xlim = x_lim,
    ylim = y_lim,
    type = "n", frame.plot = FALSE, # xaxt = "n",
    xaxs = "i", xlab = "Age (Ma)", ylab = rate_type)
  plt_vals <- calc_step_values(rows, offset = offset, max_age = max_age)
  plot_step_line_error(plt_vals, col = plt_col[[rate_type]])
  # if (rate_type == "speciation rate") {
  #   axis(1, labels = TRUE)
  #   mtext("age (ma)", side = 1, line = 1, cex = 0.8)
  #   mtext("Rates through time", 3, cex = 0.8)
  # } else {
  #   axis(3, labels = FALSE)
  # }
}
...
# reduce extinction rate y scale
cairo_pdf(file = paste0(dirs$figs, "stage_rb_net_red_plot.pdf"), width = 8, height = 5)
par(mar = c(4, 5, 0, 0) + 0.25)
rate_type <- "Net diversification rate"
rows <- rate_rows[rate_rows$rate == rate_type, ]
y_lim <- c(-0.5, 0.35)
plot(x = rows$end_times, y = rows$median,
  xlim = x_lim,
