...
### name of job
#SBATCH --job-name=TPP-ages
#SBATCH --output=TPP-ages-out
### time to stop job
...
python3 $PYRATESCRIPT $PYRATEINPUT -n 100000000 -s 5000 -j $SLURM_ARRAY_TASK_ID -out TPP-ages -A 4 -qShift ./stages -mG -pP 1.5 0

### check the likelihood
python3 $PYRATESCRIPT -mProb './pyrate_mcmc_logs/ichthyosaur_'$SLURM_ARRAY_TASK_ID'TPP-ages_G_mcmc.log' -b 200 > $SLURM_ARRAY_TASK_ID'TPP-ages-prob'
